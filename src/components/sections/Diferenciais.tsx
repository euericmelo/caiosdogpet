        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {differentials.map((d, i) => (
            <div key={i} className="flex flex-col items-center text-center gap-4 group">
              <div className="w-20 h-20 rounded-full bg-white shadow-sm border border-border flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                <d.icon className={`w-10 h-10 ${d.color}`} />
              </div>
              <p className="font-bold text-sm md:text-base text-foreground/80">{d.label}</p>
            </div>
          ))}
        </div>
      </div>
